...
def on_message(client, userdata, message):
    ...
    response = requests.post("http://localhost:59986/api/v2/resource/0x00124b00251f6ccc/sensorData", json = sensor_data)
    print("set sensorData status: ", response.status_code)
    response = requests.post("http://localhost:59986/api/v2/resource/0x00124b00251f6ccc/temperature", json = sensor_data["temperature"])
    print("set temperature status: ", response.status_code)
    response = requests.post("http://localhost:59986/api/v2/resource/0x00124b00251f6ccc/humidity", json = sensor_data["humidity"])
